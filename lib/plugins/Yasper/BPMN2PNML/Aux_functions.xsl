...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="GetPlace">
        ...
        <xsl:for-each select="../../Gateways/Gateway[ @id = $FromID ]">
            <xsl:text>pl</xsl:text><xsl:value-of select="translate(translate($FromID,'(',''),')','')"/>_<xsl:value-of
                select="$count"/>
        </xsl:for-each>
    ...
    <xsl:template name="ParseWeights">
        ...
        <xsl:variable name="position">
            <xsl:if test="boolean(../../Gateways/Gateway[ translate(translate( @id,'(',''),')','') = substring-before(substring-before($str,' '),',') ])">
                <xsl:for-each
                        select="../../SequenceFlows/SequenceFlow[ translate(translate(Target,'(',''),')','') = substring-before(substring-before($str,' '),',')  ]">
        ...
        <xsl:element name="connectionWeight">
            <xsl:attribute name="connection">
                ...
                <xsl:if test="string-length($position) > 0">_<xsl:value-of select="$position"/>
                </xsl:if>
            </xsl:attribute>
