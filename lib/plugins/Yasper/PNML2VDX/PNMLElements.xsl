...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="1.0">
    ...
    <xsl:template name="SubnetFrameWork">
        ...
        <xsl:element name="page">
            <xsl:attribute name="id">pa<xsl:value-of select="$id"/>
            </xsl:attribute>
            ...
            <xsl:choose>
                <xsl:when test="string-length($ref_outID) = 0">
                    <xsl:for-each
                        ...
                        <xsl:call-template name="ReferencePlaceFrameWork">
                            <xsl:with-param name="id">rp_ri<xsl:value-of select="$id"/>_<xsl:value-of
                        ...
                        <xsl:call-template name="ArcFrameWork">
                            <xsl:with-param name="id">ar_ri<xsl:value-of select="$id"/>_<xsl:value-of
                                    select="position()"/>
                            </xsl:with-param>
                            <xsl:with-param name="source">tr<xsl:value-of select="$id"/>
                            </xsl:with-param>
                            <xsl:with-param name="target">rp_ri<xsl:value-of select="$id"/>_<xsl:value-of
                                    select="count(preceding::SequenceFlow)"/>
                            </xsl:with-param>
                        </xsl:call-template>
                    </xsl:for-each>
                ...
                <xsl:otherwise>
                    <xsl:call-template name="ReferencePlaceFrameWork">
                        <xsl:with-param name="id">ref2<xsl:value-of select="$id"/>
                        </xsl:with-param>
                        <xsl:with-param name="ref">pl2<xsl:value-of select="$ref_outID"/>
                        </xsl:with-param>
                        <xsl:with-param name="px">300</xsl:with-param>
                        <xsl:with-param name="py">100</xsl:with-param>
                    </xsl:call-template>
                    <xsl:call-template name="ArcFrameWork">
                        <xsl:with-param name="id">ar1<xsl:value-of select="$id"/>
                        </xsl:with-param>
                        <xsl:with-param name="source">tr<xsl:value-of select="$id"/>
                        </xsl:with-param>
                        <xsl:with-param name="target">ref2<xsl:value-of select="$id"/>
                        </xsl:with-param>
                    </xsl:call-template>
                </xsl:otherwise>
            </xsl:choose>
            ...
            <xsl:choose>
                <xsl:when test="string-length($ref_inID) = 0">
                    <xsl:for-each
                        ...
                        <xsl:if test="position() = 1">
                            <xsl:call-template name="ReferencePlaceFrameWork">
                                <xsl:with-param name="id">rp_ro<xsl:value-of select="$id"/>_<xsl:value-of
                                        select="position()"/>
                                </xsl:with-param>
                            ...
                            <xsl:call-template name="ArcFrameWork">
                                <xsl:with-param name="id">ar_ro<xsl:value-of select="$id"/>_<xsl:value-of
                                        select="position()"/>
                                </xsl:with-param>
                                <xsl:with-param name="source">rp_ro<xsl:value-of select="$id"/>_<xsl:value-of
                                        select="position()"/>
                                </xsl:with-param>
                ...
                <xsl:otherwise>
                    <xsl:call-template name="ReferencePlaceFrameWork">
                        <xsl:with-param name="id">ref1<xsl:value-of select="$id"/>
                        </xsl:with-param>
                        <xsl:with-param name="ref">pl1<xsl:value-of select="$ref_inID"/>
                        </xsl:with-param>
                        <xsl:with-param name="px">50</xsl:with-param>
                        <xsl:with-param name="py">100</xsl:with-param>
                    </xsl:call-template>
                    <xsl:call-template name="ArcFrameWork">
                        <xsl:with-param name="id">arc<xsl:value-of select="$ref_inID"/>
                        </xsl:with-param>
                        <xsl:with-param name="source">ref1<xsl:value-of select="$id"/>
                        </xsl:with-param>
                        <xsl:with-param name="target">tr<xsl:value-of select="$id"/>
                        </xsl:with-param>
                    </xsl:call-template>
                ...
                <xsl:element name="toolspecific">
                    ...
                    <xsl:element name="processingTime" namespace="http://www.yasper.org/specs/epnml-1.1/toolspec">
                        <xsl:element name="mean">
                            <xsl:element name="text">
                                <xsl:value-of select="$processingTimeMean"/>
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="deviation">
                            <xsl:element name="text">
                                <xsl:value-of select="$processingTimeDeviation"/>
                            </xsl:element>
                        </xsl:element>
                    </xsl:element>
                    <xsl:element name="cost" namespace="http://www.yasper.org/specs/epnml-1.1/toolspec">
                        <xsl:element name="fixed">
                            <xsl:element name="text">
                                <xsl:value-of select="$costFixed"/>
                            </xsl:element>
                        </xsl:element>
                        <xsl:element name="variable">
                            <xsl:element name="text">
                                <xsl:value-of select="$costVariable"/>
                            </xsl:element>
                        </xsl:element>
                    </xsl:element>
                </xsl:element>
        ...
        <xsl:element name="pnml">
            <xsl:element name="net">
                <xsl:attribute name="type">
                    <xsl:text>http://www.yasper.org/specs/epnml-1.1</xsl:text>
                </xsl:attribute>
                <xsl:attribute name="id">
                    <xsl:text>tn1</xsl:text>
                </xsl:attribute>
                <xsl:element name="name">
                    <xsl:element name="text">
                        ...
                        <xsl:element name="roles" namespace="http://www.yasper.org/specs/epnml-1.1/toolspec">
                            <xsl:for-each select="FunctionalGroup/RoleNames/RoleName">
                                <xsl:element name="role">
                                    <xsl:attribute name="id">rl<xsl:value-of select="@id"/>
                                    </xsl:attribute>
                                    <xsl:element name="name">
                                        <xsl:element name="text">
                                            <xsl:value-of select="."/>
