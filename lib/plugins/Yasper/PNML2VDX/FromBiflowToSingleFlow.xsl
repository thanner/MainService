    ...
    <xsl:template match="SequenceFlow">
        <xsl:choose>
            <xsl:when test="IsBiflow = 'true' ">
                ...
                <xsl:element name="SequenceFlow">
                    <xsl:attribute name="id">(<xsl:value-of select="generate-id(.)"/>-<xsl:value-of
            ...
            <xsl:when test="$numberOfPathElements > 0">
                <xsl:element name="PathElement">
                    <xsl:attribute name="X">
                        <xsl:value-of select="Path/PathElement[$numberOfPathElements]/@X"/>
                    </xsl:attribute>
                    <xsl:attribute name="Y">
                        <xsl:value-of select="Path/PathElement[$numberOfPathElements]/@Y"/>
                    </xsl:attribute>
                </xsl:element>
                <xsl:call-template name="reversePathElementOrder">
